...
```
## Deep Dive: does Left Ventricular Mass Index increase with Age?
[DESCRIVERE QUI PERCHEè?]
...
### Checking Proportional Hazards assumptions and Linearity for Base Model
...
### Checking Proportional Hazards assumptions and Linearity for Augmented Model
...
## Augmented Model - TO DO STRATA

```{r Augmented TO DO STRATA}
augmented_model_2 <- coxph(Surv(fuyrs, status) ~ sex + age + con.cabg + creat + lv + sten.reg.mix + log.lvmi, data = data)
...
```{r Score for ROC and Calibration}
...
CVscore <- Score(list("Base"=base_model,"Augmented"=augmented_model),
              ...
              data=data,conf.int=T,
              times=seq(1,5,1),
              split.method="loob",B=100,seed=3780)
```
```{r}
CVscore
```
